    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="lg:ml-40 ml-10 space-x-8">
                    ...
                    @if($isOpen)
                        @include('livewire.matery-create')
                    @endif
            </div>
        </div>
            ...
            <table class="w-full divide-y divide-gray-200 table-auto">
              <thead class="bg-indigo-500 text-white">
                <tr class="text-left text-xs font-bold  uppercase">
                  ...
                  <td scope="col" class="px-6 py-3">creditos</td>
                  <td scope="col" class="px-6 py-3">tipo</td>
                  <td scope="col" class="px-6 py-3">etapa</td>
                  <td scope="col" class="px-6 py-3">semestre</td>
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                <tr class="text-sm font-medium text-gray-900">
                  ...
                  <td class="px-6 py-4">{{$item->creditos}}</td>
                  <td class="px-6 py-4">{{$item->tipo}}</td>
                  <td class="px-6 py-4">{{$item->etapa}}</td>
                  <td class="px-6 py-4">{{$item->semestre}}</td>

                  <td class="px-6 py-4">
                    @if ($item->history)
                        {{ $item->history->claveMateria }}
                    @else
            ...
            Swal.fire({
              ...
              confirmButtonText: 'Yes, delete it!'
                ...
                if (result.isConfirmed) {
                    //alert("del");
                    Livewire.emitTo('crud-matery','delete',id);
                    Swal.fire(
                        'Deleted!',
